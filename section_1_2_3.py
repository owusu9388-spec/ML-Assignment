...
# -------------------------
# 1) Wrap arrays in DataFrames
# -------------------------
X_train_bal_df = pd.DataFrame(X_resampled, columns=feature_names)
X_test_processed_df = pd.DataFrame(X_test_processed, columns=feature_names)

# y_resampled and y_test may be Series, numpy arrays, or similar → normalize to DataFrame
y_train_bal_df = pd.Series(y_resampled, name="target").to_frame()
y_test_df = pd.Series(y_test, name="target").to_frame()

# -------------------------
# 2) Map target labels to 0/1, dataset-aware
# -------------------------
logger.info(f"Before mapping, y_train_bal_df['target'] unique values: {y_train_bal_df['target'].unique()}")
logger.info(f"Before mapping, y_test_df['target'] unique values: {y_test_df['target'].unique()}")

if DATASET == "telco":
    # Telco churn usually uses 'Yes'/'No'
    mapping = {"No": 0, "Yes": 1}
    y_train_bal_df["target"] = y_train_bal_df["target"].map(mapping)
    y_test_df["target"] = y_test_df["target"].map(mapping)
    logger.info("[Export] Applied Telco label mapping: 'No'→0, 'Yes'→1")

elif DATASET == "online":
    # Online shoppers Revenue is often boolean → convert boolean to int (False→0, True→1)
    y_train_bal_df["target"] = y_train_bal_df["target"].astype(int)
    y_test_df["target"] = y_test_df["target"].astype(int)
    logger.info("[Export] Converted Online 'Revenue' from bool to int (False→0, True→1)")

else:
    # Generic fallback: try to convert to int if possible, otherwise leave as-is
    try:
        y_train_bal_df["target"] = y_train_bal_df["target"].astype(int)
        y_test_df["target"] = y_test_df["target"].astype(int)
        logger.info("[Export] Generic label conversion to int applied.")
    except Exception as e:
        logger.warning(f"[Export] Could not generically convert labels to int: {e}. Leaving labels as-is.")

logger.info(f"After mapping, y_train_bal_df['target'] unique values: {y_train_bal_df['target'].unique()}")
logger.info(f"After mapping, y_test_df['target'] unique values: {y_test_df['target'].unique()}")


# -------------------------
# 3) Export training-set (balanced)
# -------------------------
X_train_bal_path = f"{output_dir}/{DATASET}_X_train_bal.csv"
y_train_bal_path = f"{output_dir}/{DATASET}_y_train_bal.csv"

X_train_bal_df.to_csv(X_train_bal_path, index=False)
y_train_bal_df.to_csv(y_train_bal_path, index=False)

logger.success(f"[Export] Saved balanced training features to: {X_train_bal_path}")
logger.success(f"[Export] Saved balanced training labels to:   {y_train_bal_path}")


# -------------------------
# 4) Export test-set (processed but NOT balanced)
# -------------------------
X_test_path = f"{output_dir}/{DATASET}_X_test_processed.csv"
y_test_path = f"{output_dir}/{DATASET}_y_test.csv"

X_test_processed_df.to_csv(X_test_path, index=False)
y_test_df.to_csv(y_test_path, index=False)

logger.success(f"[Export] Saved processed test features to: {X_test_path}")
logger.success(f"[Export] Saved test labels to:             {y_test_path}")


# -------------------------
# 5) Export feature names
# -------------------------
feat_path = f"{output_dir}/{DATASET}_final_feature_names.csv"
pd.Series(feature_names, name="feature_name").to_csv(feat_path, index=False)
logger.success(f"[Export] Saved final feature names to: {feat_path}")


# -------------------------
# 6) Save preprocessor
# -------------------------
preproc_path = f"{output_dir}/{DATASET}_preprocessor.pkl"
joblib.dump(preprocessor, preproc_path)
logger.success(f"[Export] Saved fitted preprocessor to: {preproc_path}")

logger.success(f"All Section 4 files exported to folder: {output_dir}")
